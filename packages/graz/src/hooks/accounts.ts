...
import type { ConnectArgs, ConnectResult } from "../actions/accounts";
import { connect, disconnect, getOfflineSigners, reconnect } from "../actions/accounts";
 ...
 * ```
 */
export const useAccounts = ({ onConnect, onDisconnect }: UseAccountsArgs = {}) => {
  const _accounts = useGrazSessionStore((x) => x.accounts);
  ...
  useEffect(() => {
    return useGrazSessionStore.subscribe(
      ...
      (stat, prevStat) => {
        if (stat === "connected") {
          const { accounts, activeChain } = useGrazSessionStore.getState();
...
export const useBalances = (bech32Address: string): UseQueryResult<Coin[]> => {
  ...
  const account = Array.from(accounts.values()).find((acc) => acc.key.bech32Address === bech32Address);
  if (!account) throw new Error("account is not defined");
  const address = account.key.bech32Address;
 ...
 * @see {@link connect}
 */
export const useConnectMultiple = ({ onError, onLoading, onSuccess }: UseConnectChainArgs = {}) => {
